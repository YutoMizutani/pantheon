...
> 出典（一次ソース）: ラムダノート代表・鹿野桂一郎 氏の Gist
> https://gist.github.com/k16shikano/fd287c3133457c4fd8f5601d34aa817d
> 紹介記事（テクノエッジ）: https://www.techno-edge.net/article/2026/06/22/5209.html
> 取得日: 2026-06-23。本文は Gist から逐語で取り込んだもの。
>
> 適用範囲の注意: これは「出版する日本語技術文書」のレジスタに較正された規範。
> エージェントの推論ログ・短い進捗報告・完了報告には自動適用しない（この skill の
> description トリガ＝文書/記事/草稿を書く・推敲するとき に限る）。とくに
> 「〜かもしれない」等の hedge 禁止は本規範でも「根拠なく弱める場合だけ」と限定されており、
> 根拠付きの確度表明（`[確度/根拠/未検証]` marker）はこのリポジトリの別規範として保持する。
