 ...
 *
 * ==== 規範 / hook との関係 ====
 *   対応する規範: CLAUDE.md「バグ修正フロー: RED ファースト原則」
 *   対応する hook: .claude/hooks/block_red_first_violation.py (reactive deny)
...
// args は object として渡される想定だが、harness/Skill ブリッジは string で渡す.
// JSON string なら parse、bare string (非 JSON) は report そのものとして受理する
// (feature-loop.js と同じ graceful 契約)。throw すると Skill 経由の初回起動が
// 必ず launch→silent fail→re-launch の往復になる (2026-06-11 session 10412678 で実害).
let _args = args
if (typeof _args === 'string') {
  const raw = _args
  try {
    const parsed = JSON.parse(raw)
    _args = parsed && typeof parsed === 'object' && !Array.isArray(parsed) ? parsed : { report: raw }
  } catch (e) {
    _args = { report: raw }
  }
}
