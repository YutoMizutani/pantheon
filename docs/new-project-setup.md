...
├── app/ | apps/     # 実アプリ本体（下記「app/ と apps/ の使い分け」参照。任意 — 持つときのみ）
...
### `app/` と `apps/` の使い分け（個数で決める）

アプリ本体を置くディレクトリは **アプリの個数** で単複を決める（環境横断の命名規約）:

- **1 アプリ → `app/`**（単数）。コードは `app/` 直下にフラット配置してよい。
- **2 アプリ以上 → `apps/`**（複数）。各アプリを `apps/<app-name>/` のサブディレクトリに分ける。
- 1 個から 2 個目を足すタイミングで `app/` → `apps/` に改名し、参照（Taskfile / pm2 の `ENV_FILE` / スクリプト / doc）を併せて更新する。
- リポジトリ直下に置く横断アプリ（例: `app/channel-watcher`）も同じ規約に従う。
