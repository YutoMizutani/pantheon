...
├── apps/            # 実アプリ本体（apps/<app-name>/ に配置。下記「apps/ 規約」参照。任意 — 持つときのみ）
...
アプリ本体は **常に複数形 `apps/`** の下に `apps/<app-name>/` のサブディレクトリで置く（環境横断の命名規約）。アプリが 1 個でも `app/`（単数・フラット）にはしない:

- プロジェクトは時間とともにアプリが増える。最初から `apps/<app-name>/` に統一しておけば、「2 個目を足す時に `app/` → `apps/` へ改名し参照（Taskfile / pm2 の `ENV_FILE` / スクリプト / doc）を更新する」という手戻りが発生しない。
- 例: `apps/dashboard/`, `apps/channel-watcher/`。
- リポジトリ直下に置く横断アプリ（例: `apps/channel-watcher`）も同じ規約に従う。
