...
| 上位層への昇格（規範の promotion） | project 固有 → `projects/<X>/CLAUDE.md`・横断×ユーザー固有 → `CLAUDE.local.md`・横断×環境汎用 → `.claude/rules/common/` | target から導出 | 人間ゲート（queue 経由のみ）+ ルート `CLAUDE.md` は対象外 |
