 ...
 * schema の source_type enum (datamine/market_api 等)・失敗カタログ・subagent プロンプトの注意節は
 ...
 *
 * ==== 規範 / hook との関係 ====
 *   対応する規範: docs/effective-rules.md「一次ソース監査」エントリ
...
//   project?     : string   プロジェクト名 (context 注入用. "my-game" 等)
//   budget?      : string   調査範囲 / 予算制約の宣言 ("datamine + wiki 最大10fetch" / "現在の保有値=41" 等)
...
if (typeof _args === 'string') {
  try {
    ...
    throw new Error(`web-research workflow: args was a string but not valid JSON: ${e.message}`)
  }
}
...
5. budget に予算/数量制約 (現在の保有値 / 上限数等) があれば **Σ必要 ≤ 現resource を算術で検算** し budget_check に書く. 無ければ "N/A"
