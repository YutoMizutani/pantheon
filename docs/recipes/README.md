...
> **他環境への配布の単位は recipes であって、原環境で獲得した memory / 校正定数 / 規範そのものではない。**
> 後者は原環境の失敗観測に較正されており、効力の根拠が運べない（[[feedback_extraction_boundary_is_dependency_closure_not_directory]]）。
> 配布するのは規範の実体でなく「同種の失敗を観測したら自環境で再導出する手順」。機構と recipes は運べるが、規範の中身は各環境が自分の失敗観測から獲得する。
