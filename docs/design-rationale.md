...
| 層 | 定義 | 扱い |
|---|---|---|
| **機構** | 環境非依存を**目指す**層。ループ自体（検出 / reflection / telemetry / memory 衛生 / 棚卸し）。現状は原環境の絶対パスが一部混入しており、移植時は校正が要る（解消は段階的） | 配線して出荷（`.claude/settings.json`） |
...
**「産物（content）が他環境へ運べない」のは欠陥でなく設計である。** 各運用者の規範は自環境の失敗観測に較正されているため、content の非可搬性は当然の帰結で、これを可搬化する機構を作る必要はない。運べるのは機構と recipes だけで十分。矛盾していたのは「フレーム層はどこでも通用する」という claim の側であって、非可搬性そのものではない。

**採用は2モード。** (a) 部品だけ採る → recipes を取り、配線済みループは入れない。(b) ループごと採る → opt-in で `signals.json` を埋める。**既定（`signals.json` が空）ではループは一切発火せず、昇格も提案も出ない。** フレームは自動で奉仕する装置でなく、運用者が起動して操作する機構で、既定で黙っているのが正常。clone 直後の Claude が未 setup に気づいても自発的に整備を提案しないのはこのため（初回セットアップは明示依頼でのみ起動する）。
