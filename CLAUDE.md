...
全プロジェクト横断で守らせたい**ユーザー固有の原則**は、ここではなく `CLAUDE.local.md` の「全体方針」節に書く（書き方と実例は [CLAUDE.local.md.example](CLAUDE.local.md.example)）。この節（フレーム層）に残すのは、どの環境でも*再生できる*上記の機構だけ（較正は移植先で各自行う）。
