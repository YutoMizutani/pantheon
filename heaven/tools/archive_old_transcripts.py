...
配線 (machine 固有 — 別環境では path/Label を調整):
  実体は ~/Library/LaunchAgents/com.ym.claude-transcript-archiver.plist (週次 日曜 04:30 --apply)。
  その再現可能な写しを同階層 heaven/tools/com.ym.claude-transcript-archiver.plist に commit 済み。
  新 machine への配線:
    cp heaven/tools/com.ym.claude-transcript-archiver.plist ~/Library/LaunchAgents/
    launchctl load ~/Library/LaunchAgents/com.ym.claude-transcript-archiver.plist
  3-way 結合 (この script 180d / plist schedule / settings.json cleanupPeriodDays=200) を崩さないこと
  — 200d > 180d なので archiver が先に走り cleanup 削除は起きない。どれか変えたら 3 箇所を整合させる。
"""
import argparse
import gzip
import os
